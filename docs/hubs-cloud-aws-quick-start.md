...
2. Register or setup any domains in AWS Route 53, you'll need at least 2 domains. For example: `myhub.com` and `myhub.link`. See [Domain Recipes](./hubs-cloud-aws-domain-recipes.html) for more info.
3. Review relevant docs:
   ...
   - [Cost Information](hubs-cloud-aws-costs)
...
8. In specify stack details:
   ...
   - For these parameters use [Domain Recipes](hubs-cloud-aws-domain-recipes) for guidance: `Site Domain Name`, `Site is Set Up On Route 53`, `Internal Domain`, `Short Link Domain`, `Outgoing Email Domain`, and `Outgoing Email Subdomain Prefix`
     - Double check for no typos!
   - Choose your `KeyPair` from Before Creating the Stack: Step 4
   - If you are using an existing domain not on AWS Route 53, you'll need to perform a few extra steps - See [Using an existing domain](hubs-cloud-aws-existing-domain)
   - Choose a setting for `Restrict SSH Access`
   - Review the other options, or keep the defaults. You can update most of these later via a [Stack Update](hubs-cloud-aws-updating-the-stack)
9. Select **"Next"**
10. Agree to Terms of Service checkboxes
11. Wait 20-30 minutes for the stack to complete deploying
    - Any issues? Check out [AWS Troubleshooting](hubs-cloud-aws-troubleshooting) for solutions to common problems.
...
3. Proceed with process to add scenes and avatars in the [Getting Started with Hubs Cloud](hubs-cloud-getting-started) guide!

**Congrats you've successfully set everything up!**

#### Any issues deploying?

Check out [AWS Troubleshooting](hubs-cloud-aws-troubleshooting) for solutions to common problems.
